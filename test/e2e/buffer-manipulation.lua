...
describe('Buffer Manipulation', function()
	...
	it('Clears extmarks when moving line out of injected language', function()
		...
		assert.remote(nvim).has_extmarks_at(3, 5, 'lua')
		...
		assert.remote(nvim).Not.has_extmarks_at(4, 5, 'lua')
	end)
	...
	it('Preserves extmarks upon multi-line substitution', function()
		...
		nvim:exec_lua('vim.treesitter.start()', {})
		assert.remote(nvim).has_extmarks_at(1, 1, 'lua', 'RainbowDelimiterYellow')
		assert.remote(nvim).has_extmarks_at(3, 1, 'lua', 'RainbowDelimiterYellow')
...
[[print {{},
	{},
		...
		assert.remote(nvim).has_extmarks_at(0, 7, 'lua', 'RainbowDelimiterYellow')
		assert.remote(nvim).has_extmarks_at(3, 1, 'lua', 'RainbowDelimiterYellow')
	end)
	...
	{a = 1, b = 2},
		...
		before_each(function()
			...
			nvim:buf_set_option(0, 'filetype', 'lua')
			nvim:exec_lua('vim.treesitter.start()', {})
			assert.remote(nvim).has_extmarks_at(1, 01, 'lua', 'RainbowDelimiterYellow')
			assert.remote(nvim).has_extmarks_at(1, 14, 'lua', 'RainbowDelimiterYellow')
		end)

		it('Properly highlights after inserting one line', function()
			nvim:feedkeys('ggjyyp', 'n', false)

			assert.remote(nvim).has_extmarks_at(2, 01, 'lua', 'RainbowDelimiterYellow')
			assert.remote(nvim).has_extmarks_at(2, 14, 'lua', 'RainbowDelimiterYellow')
		end)
